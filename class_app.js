...
function handleListEdit(dateStr, no, field, value) {
  ...
  if (field === 'status') {
    if (value === 'present') {
      ...
      state.attendance[dateStr][no] = {
        status: 'infirmary',
        // 初期は何もチェックされない状態(ユーザーが時限を選択)
        periods: Array.isArray(cur.periods) ? cur.periods.slice() : []
      };
    } else {
...
function saveStudentModal() {
  ...
  if (ctx.status === 'present') {
    ...
    // 時限はユーザーが手動で選択する(自動チェックしない)
    const periods = Array.isArray(ctx.periods) ? ctx.periods.slice().sort((a,b)=>a-b) : [];
    state.attendance[ctx.dateStr][ctx.stuNo] = { status: 'infirmary', periods };
  } else {
